...
onready var dialogue_bubble = $Granny/DialogueBubble
...
func _on_Granny_interact():
	...
	if (item):
		if (item == preload("res://Island/Items/mail.tres")):
			Game.flags.add(flag)
	...
	if (Game.flags.has(flag)):
		dialogue_bubble.show_dialogue(["没想到老头子的船票寄过来了，谢谢你"])
	else:
		dialogue_bubble.show_dialogue([
			"我年纪大了，很多事情想不起来了。",
